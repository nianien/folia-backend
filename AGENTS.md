...
This repository is a Python backend prototype. `src/frontpage_pipeline/` contains the data pipeline package, `config/*.toml` contains runtime configuration, `docs/` contains product and technical design documents, `tests/` contains standard-library `unittest` coverage, and `data/` is reserved for local SQLite files.
...
Prefer clear, boring names that match the spec: `articles`, `clusters`, `layout`, `fetcher`, `extractor`, `dedupe`, `scorer`, and `ranker`.
